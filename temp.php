...
$ok = true; // Set $ok to true by default
// Remove the foreach loop and the die statementforeach ($telegram_ip_ranges as $telegram_ip_range) if (!$ok) {
...
// ...
$channel='@munisa_onlinesavdo_wechat';
$resuser = bot('getChatMember', [
    'chat_id' => $channel,
    'user_id' => $from_id
]);
if ($resuser['result']['status'] == 'member' && $resuser['ok']) {
    // Foydalanuvchi kanalga obuna bo'lgan bo'lsa
    // Bu joyda xabar jo'natish kodlari yoziladi
} else {
    // Foydalanuvchi kanalga obuna bo'lmagan yoki kanaldan chiqarilgan bo'lsa
    bot('sendMessage',[
        'chat_id'=>$chat_id,
